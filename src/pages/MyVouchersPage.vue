...
const activeTab = ref<Tab>('unused')
const sort = ref<'new'|'expiring'>('new')
const page = ref(1)
const pageSize = ref(6)

const coupons = ref<Coupon[]>(generateDemo())
...
function setSort(s:'new'|'expiring') { sort.value = s; page.value = 1 }

function generateDemo(): Coupon[] {
  const currs = ['USD','GBP','EUR','CAD'] as const
  const res: Coupon[] = []
  const now = new Date()
  let id = 1
  for (let i=0;i<18;i++) {
