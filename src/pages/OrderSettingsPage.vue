...
const platOrderSet = ref<'1' | '0'>('0')

// 自动上传跟踪号
const autoUploadRaw = ref<BoolString>('false')
const autoUpload = computed<boolean>({
...
interface StoreRule {
  ...
  lastUpdateUnit: 1 | 2 | 3
  allowNotShipped: boolean
  selected: boolean
}

// 与附件一致：表格可为空
const stores = ref<StoreRule[]>([])

const normalizeStoreLimit = (s: StoreRule) => {
...
const toggleService = (code: keyof typeof services.value) => {
