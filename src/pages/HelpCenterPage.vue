            ...
            <div class="div-4">
              <div v-if="filteredContent.length > 0">
                ...
                <div v-for="faqSection in filteredContent" :key="faqSection.title" class="div-5">
                  ...
                  <ul class="ul-15">
                    <li v-for="(item, idx) in faqSection.items" :key="idx" class="li-2">
                      <a :href="getFaqDetailLink(item.url, item.title)" class="a-45">{{ item.title }}</a>
                    </li>
                  </ul>
                </div>
              </div>
      ...
      <div class="hc-container hc-footer__inner">
        <div class="hc-footer__col">
          ...
          <ul>
            ...
            <li><a href="/contact-us">联��我们</a></li>
          </ul>
        </div>
        <div class="hc-footer__col">
          <h3>客户服务</h3>
...
import { helpCenterNav, type NavItem } from '@/data/helpCenterNav'
import { faqData, getFaqById } from '@/data/faqData'
import { useRouter } from 'vue-router'

const router = useRouter()

interface PartnerItem { alt: string; src: string }
...
// 展开折叠状态（默认展开第一个分类）
const openKeys = ref<Set<string>>(new Set(['platform-usage']))
...
// 导航数据
const navData = helpCenterNav

// 选中的子项ID
const selectedChildId = ref<string>('platform-rules')
...
function handleSearch() {
  // 搜索逻辑已通过计算属性 filteredContent 处理
}

// 从 URL 提取 FAQ ID 并返回详情页链接
function getFaqDetailLink(url: string, title?: string): string {
  // 从 URL 中提取 ID，如: https://www.saleyee.com/faq/hp746821.html -> hp746821
  const match = url.match(/\/faq\/([a-zA-Z0-9]+)\.html/)
  if (match && match[1]) {
    const id = match[1]
    // 如果本地 faqData 中存在该 id，则返回详情链接
    if (getFaqById(id)) {
      return `/faq/${id}`
    }
  }
  // 如果 URL 中的 id 不存在，尝试通过标题匹配 faqData
  if (title) {
    const found = faqData.find(f => f.question && f.question.trim() === title.trim())
    if (found) {
      return `/faq/${found.id}`
    }
  }
...
.p-2, .p-3 { color: #333; font-size: 16px; margin-bottom: 4px; }
.ul-15, .ul-16 { margin-left: 30px; padding: 0; }
.li-2, .li-3, .li-4, .li-5, .li-6, .li-7, .li-8, .li-9, .li-10 { list-style: disc; color: #999; }
.a-45, .a-46, .a-47, .a-48, .a-49, .a-50, .a-52, .a-53, .a-54 { color: #333; text-decoration: none; line-height: 36px; }
.a-51 { display: inline-block; padding-left: 30px; line-height: 36px; color: #333; text-decoration: none; }
.div-10 { clear: both; }
