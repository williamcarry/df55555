...
const productId = computed(() => {
  const idFromRoute = (route.params.id || route.query.id) as string | undefined
...
const activeTab = ref<'dropship' | 'factory'>('dropship')
...
function selectThumbnail(url: string, index?: number) {
  selectedImage.value = url
  if (typeof index === 'number') selectedIndex.value = index
}

function handleThumbnailMouseMove(e: MouseEvent) {
  const wrapper = (e.currentTarget as HTMLElement)
  const scrollContainer = wrapper.querySelector('.thumbnail-scroll-container') as HTMLElement
...
function handlePublish(platform: 'amazon' | 'shopify') {
