...
const activeTab = ref<'shipping' | 'billing'>('shipping')
...
function handleEditAddress(row: Address) {
  formData.recipient = row.recipient
  formData.phone = row.phone
  formData.email = (row as any).email || ''
  formData.address = row.address
  formData.address2 = (row as any).address2 || ''
...
function handleSelectionChange(selection: Address[]) {
