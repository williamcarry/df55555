...
<template>
  <div class="bg-white rounded-lg border border-slate-200 p-6">
    ...
    <div class="bg-[#fcf8e3] border border-[#faebcc] rounded mt-3 p-3 flex items-start">
      ...
      <p class="text-[#a77200] leading-6">Payoneer账号最多可添加5个，您可先删除不用账号，待客户经理审核通过后，再添加新账号；绑定账号请���写您的支付账户（邮箱/手机号）；空中云汇不支持授权绑定账号。</p>
    </div>
    ...
    <el-dialog v-model="addVisible" title="添加账号" width="480">
      <div class="space-y-4">
        ...
        <div class="flex items-center">
          <span class="inline-block w-[90px] text-right mr-3">账户</span>
...
interface Row { id:number; type:'Payoneer'|'Airwallex'; account:string; status:'未绑定'|'已绑定'|'未激活'; createdAt:string }

const allRows = ref<Row[]>(generateDemo())
...
const filteredRows = computed(()=> allRows.value.filter(r =>
  (!filters.paymentType || r.type === filters.paymentType as Row['type']) &&
  (!filters.bindStatus || r.status === filters.bindStatus as Row['status'])
))
...
function removeRow(id:number){ allRows.value = allRows.value.filter(r=>r.id!==id) }
function bind(r:Row){ r.status = '已绑定' }
function unbind(r:Row){ r.status = '未绑定' }

const addVisible = ref(false)
const form = reactive<{ type:''|'Payoneer'|'Airwallex'; account:string }>({ type:'', account:'' })
...
function generateDemo(): Row[] {
  const list: Row[] = []
  const types: Array<Row['type']> = ['Payoneer','Airwallex']
  const statuses: Array<Row['status']> = ['未绑定','已绑定','未激活']
  const now = new Date()
  for(let i=0;i<23;i++){
