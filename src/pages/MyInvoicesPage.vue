...
interface BillRow { id:number; selected:boolean; orderNo:string; payTradeNo:string; amount:number; fee:number; currency:'USD'|'GBP'|'EUR'|'CAD'; dealType:'充值'|'消费'|'提现'; method:string; account:string; time:string }

const allRows = ref<BillRow[]>(generateDemoRows())
...
function generateDemoRows(): BillRow[] {
  const currencies = ['USD','GBP','EUR','CAD'] as const
  const types = ['充值','消费','提现'] as const
