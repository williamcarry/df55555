...
import type { NavItem, FAQ } from '@/data/helpCenterNav'
...
// 展开折叠状态（默认展开第一个分类��
const openKeys = ref<Set<string>>(new Set([operationGuideNav[0]?.id || '']))
...
// 导航数据
const navData: NavItem[] = operationGuideNav

// 选中的子项ID（默认第一个子项）
const selectedChildId = ref<string>(operationGuideNav[0]?.children?.[0]?.id || '')
...
function getFaqLink(item: FAQ): string {
