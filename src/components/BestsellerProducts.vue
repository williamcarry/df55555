...
const themes = ref<BestsellerTheme[]>([
  {
    title: '热销爆款',
    products: [
      {
...
function extractItemId(href: string): string {
  const m = href?.match(/\/item\/(\d+)/)
  return m ? m[1] : ''
}
function productLink(p: Product): string {
  // @ts-ignore allow missing href
  const raw = (p as any).href || ''
  // @ts-ignore allow optional id
  const id = (p as any).id || extractItemId(raw)
  return id ? `/item/${id}` : raw || '#'
}
function linkTarget(p: Product): string | undefined {
...
<template>
  <section class="w-full" style="background-color: #F2F3F7;">
    ...
    <div class="w-full text-center py-11" style="background-color: #F2F3F7;">
      <h2 class="text-3xl font-bold text-slate-800">家居爆款热销精选</h2>
    </div>
